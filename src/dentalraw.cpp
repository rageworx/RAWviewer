...
DentalRAWReader::DentalRAWReader()
 : raw_loaded(false),
   pixel_min_level(0),
   pixel_max_level(0),
...
DentalRAWReader::DentalRAWReader( const TCHAR* raw_file )
 : raw_loaded(false),
   pixel_min_level(0),
   pixel_max_level(0),
   img_height(DEF_RAW_I_HEIGHT),
...
bool DentalRAWReader::Load( const TCHAR* raw_file )
{
    fstream rfstrm;
...
bool DentalRAWReader::Reload( const TCHAR* raw_file )
{
    Unload();
    return Load( raw_file );
}

bool DentalRAWReader::Reload()
{
    return Reload( _TCM2W(raw_file_name.c_str()) );
}

void DentalRAWReader::Unload()
{
    raw_loaded = false;
...
void DentalRAWReader::SetUserScale( VatechRAWUserScaleIF* ptr )
{
    userscaler = ptr;
}

bool DentalRAWReader::Get8bitDownscaled( vector<unsigned char> &byte_arrays, DownscaleType dntype )
{
    if ( raw_loaded == false )
        return false;
...
bool DentalRAWReader::Get16bitRawImage( std::vector<unsigned short> &word_arrays )
{
    if ( raw_loaded == false )
        return false;
...
bool DentalRAWReader::GetWeights( std::vector<int> &weight_arrays )
{
    if ( raw_loaded == false )
        return false;
...
bool DentalRAWReader::GetAnalisysReport( WeightAnalisysReport &report )
{
    ...
    // # phase 04
    // get wide count.
    report.threshold_wide     = max_weight_wide - min_weight_wide;
...
bool DentalRAWReader::Get16bitThresholdedImage( WeightAnalisysReport &report,  vector<unsigned short> &word_arrays )
{
    ...
    float normf = DEF_PIXEL_WEIGHTS / float(report.threshold_wide);
...
bool DentalRAWReader::Get8bitThresholdedImage( WeightAnalisysReport &report, std::vector<unsigned char> &byte_arrays )
{
    if ( report.timestamp == 0 )
        return false;
...
void DentalRAWReader::resetWeights()
{
    pixel_weights.clear();
